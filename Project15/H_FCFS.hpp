#ifndef __H_FCFS_HPP__
#define __H_FCFS_HPP__
...
class ComparisonClass { public: bool operator() (SProcess* a, SProcess* b) { return (a->arrive>b->arrive); } };
...
public:  C_FCFS() { process = vector<SProcess*>(); last_index = 0; }
...
private:
	void generate_process(int n, vector<SWaiting*>&waiting) {
		...
		for (int i = 0; i < n; i++) {
			burst = int(rand() % 10 + 1);
			process.push_back(new SProcess(last_index, burst, int(rand() % 10)));
...
public:  vector<SWaiting*>  FCFS(int n) {
	int time = 0;
